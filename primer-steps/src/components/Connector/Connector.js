...
const Connector = React.memo(({ index, isCompletedStep, children, isLastStep, hasLabel, size = 'md', }) => {
    const { isVertical, isLabelVertical, widths, stepSizes } = useStepsContext();
    ...
    const getMargin = () => {
        if (isVertical)
            return `calc(${stepIconContainerSizes[size].width} / 2)`;
        if (!hasLabel)
            return 2;
    };
    const styles = React.useMemo(() => {
        ...
        if (isLabelVertical) {
            return {
                ...
                right: `calc((${widths?.[index + 1]}px - ${stepIconContainerSizes[size].width}) / -2 + 8px)`,
            };
        }
        return base;
    }, [
    ...
    return (_jsx(Box, { sx: {
            ...
            transitionDuration: 'normal',
            ...styles,
        }, "data-highlighted": dataAttr(isCompletedStep), children: isVertical && children }));
});
